...
class GrafanaSourceConsumer(ConsumerBase):
    ...
    def add_source(self, source_data, rel_id=None):
        ...
        rel.data[self.charm.app]["sources"] = source_data
    ...
    def remove_source(self, rel_id=None):
        ...
        rel.data[self.charm.app].pop("sources", None)
        source = self._stored.sources.pop(rel_id, None)
    ...
    def removed_source_names(self):
        ...
        sources = []
        for source in self._stored.source_to_delete:
            sources.append(source)

        return sources

    def port(self, rel_id=None):
        """Return the port which the grafana instance is listening on"""
        rel = self.framework.model.get_relation(self.relation_name, rel_id)
...
class GrafanaSourceProvider(ProviderBase):
    ...
    def on_grafana_source_relation_changed(self, event):
        ...
        datasource_fields = {
            ...
            config.REQUIRED_DATASOURCE_FIELDS | config.OPTIONAL_DATASOURCE_FIELDS
        }

        missing_fields = [
            field for field in config.REQUIRED_DATASOURCE_FIELDS
