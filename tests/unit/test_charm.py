...
def test_datasource_config_is_updated_by_raw_grafana_source_relation(
    ctx: Context, base_state, peer_relation: PeerRelation
):
    ...
    updated_peer_relation = PeerRelation(
        "grafana", local_app_data=updated_peer_relation.local_app_data
    )
    ...
    with ctx(ctx.on.relation_departed(datasource), state) as mgr:
        ...
        assert config.get("deleteDatasources") == [
...
def test_datasource_timeout_value_is_overridden_by_config_if_smaller(
...
def test_bare_charm_has_no_subpath_set_in_layer(ctx, base_state, event):
    ...
    with ctx(event, base_state) as mgr:
        ...
        assert (
            pebble_layer.to_dict()["services"]["grafana"]["environment"]["GF_SERVER_ROOT_URL"]
...
def test_ingress_relation_sets_options_and_rel_data(ctx: Context, base_state, peer_relation):
    ...
    state = replace(
        base_state, relations={ingress_rel, peer_relation}, model=Model(name="testmodel")
    )
...
def test_config_is_updated_with_authentication_config(ctx, base_state, peer_relation):
    ...
    auth_rel = Relation(
        "grafana-auth",
        remote_app_name="auth_provider",
        remote_app_data={"auth": json.dumps(example_auth_conf)},
    )
...
def test_primary_sets_correct_peer_data(ctx, base_state):
    ...
    state = replace(
        base_state,
        planned_units=2,
        networks={Network("grafana", bind_addresses=[BindAddress([Address("1.2.3.4")])])},
    )
...
def test_replicas_get_correct_environment_variables(ctx, base_state, event):
    # GIVEN a grafana app with 2 units
    updated_peer_relation = PeerRelation(
        "grafana", local_app_data={"replica_primary": json.dumps("1.2.3.4")}
    )
